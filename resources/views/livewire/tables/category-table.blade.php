        ...
        <ul class="pagination m-0">
            @if ($categories->lastPage() > 1)
                <li class="page-item {{ $categories->onFirstPage() ? 'disabled' : '' }}">
                    <a wire:click="previousPage" class="page-link" href="#" aria-label="prev">prev</a>
                </li>

                @for ($i = 1; $i <= $categories->lastPage(); $i++)
                    <li class="page-item {{ $categories->currentPage() == $i ? 'active' : '' }}">
                        <a wire:click="gotoPage({{ $i }})" class="page-link" href="#">{{ $i }}</a>
                    </li>
                @endfor

                <li class="page-item {{ $categories->currentPage() == $categories->lastPage() ? 'disabled' : '' }}">
                    <a wire:click="nextPage" class="page-link" href="#" aria-label="next">next</a>
                </li>
            @endif
        </ul>
    </div>
